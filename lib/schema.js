...
const processSchema = (subject, id, schemaVersion, pointer, anchors, recursiveAnchors) => {
  if (isObject(subject)) {
    ...
    const commentToken = getConfig(schemaVersion, "commentToken");
    if (typeof subject[commentToken] === "string") {
      delete subject[commentToken];
    }
...
const followReferences = (doc) => {
  const jsrefToken = getConfig(doc.schemaVersion, "jsrefToken");
  const dynamicJsrefToken = getConfig(doc.schemaVersion, "dynamicJsrefToken");

  if (isObject(doc.value) && internalJref in doc.value) {
    return get(doc.value[internalJref], doc);
  } else if (jsrefToken && typeof doc.value === "string" && doc.pointer.endsWith(`/${jsrefToken}`)) {
    return get(doc.value, doc);
  } else if (dynamicJsrefToken && typeof doc.value === "string" && doc.pointer.endsWith(`/${dynamicJsrefToken}`)) {
    return get(doc.value, doc, true);
  } else {
    return doc;
  }
};
