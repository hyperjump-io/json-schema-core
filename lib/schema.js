...
const add = (schema, url = "", defaultSchemaVersion = "") => {
  ...
  let vocabulary;
  if (isObject(schema) && "$vocabulary" in schema) {
    configAlias[id] = schemaVersion;
    vocabulary = schema["$vocabulary"];
    delete schema["$vocabulary"];

    (getConfig(schemaVersion, "mandatoryVocabularies") || []).forEach((vocabularyId) => {
  ...
  const anchors = {};
  schemaStore[id] = {
    schemaVersion: id in configAlias ? id : schemaVersion,
