...
const get = async (url, contextDoc = nil, recursive = false) => {
  ...
  if (isObject(docValue) && "$__jref" in docValue) {
    return get(docValue["$__jref"], doc, recursive);
  } else if (typeof docValue === "string" && doc.pointer.endsWith("/$ref")) {
    return get(docValue, doc, recursive);
  } else if (typeof docValue === "string" && doc.pointer.endsWith("/$recursiveRef")) {
...
module.exports = { add, get, markValidated, uri, value, step, sibling, entries, map };
